	<?php
		...
		*/
		$aColumns = array( 'nm_obat','stok_obat','hpp_obat','hjual_obat','id_obat' ); //Kolom Pada Tabel
...
//filter
		if (!empty($aFilteringRules)) {
			$sWhere = " WHERE jenis_obat='OBAT' and ".implode(" AND ", $aFilteringRules);
			} else {
			$sWhere = " WHERE jenis_obat='OBAT' ";
		}
		...
		*/
		...
		while ( $aRow = $rResult->fetch_assoc() ) {
			...
			if ($level_user=='user') {
							$row = array( $aRow['nm_obat'],$aRow['stok_obat'], 'khusus admin',
											number_format($aRow['hjual_obat'], 0, ".", ".") ,'khusus admin');
			}
			else{
							$row = array( $aRow['nm_obat'],$aRow['stok_obat'], number_format($aRow['hpp_obat'], 0, ".", "."),
